...
def _get_subseasonal_univar_gen_func() -> tuple[GenFunc, Features]:
    precip = data_loaders.get_ground_truth("us_precip")
    tmp2m = data_loaders.get_ground_truth("us_tmp2m")
    tmin = data_loaders.get_ground_truth("us_tmin")
    tmax = data_loaders.get_ground_truth("us_tmax")
    ...
    def gen_func() -> Generator[dict[str, Any], None, None]:
        for lat, lon in lat_lon:
            ...
            target_dict = {
                0: lat_lon_precip[11323:]["precip"].to_numpy(),
                1: lat_lon_tmp2m["tmp2m"].to_numpy(),
                2: lat_lon_tmin["tmin"].to_numpy(),
                3: lat_lon_tmax["tmax"].to_numpy(),
            }
            for i in range(4):
                yield dict(
                    item_id=f"{lat}_{lon}_{i}",
                    start=lat_lon_precip[11323:].index[0],
...
class SubseasonalUnivarDatasetBuilder(LOTSADatasetBuilder):
    dataset_list = [
        "subseasonal_univar",
        "subseasonal_precip",
    ]
    ...
    def build_dataset(self, dataset: str):
        gen_func, features = {
            "subseasonal_univar": _get_subseasonal_univar_gen_func,
            "subseasonal_precip": _get_subseasonal_precip_gen_func,
        }[dataset]()
