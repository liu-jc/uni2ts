...
class ERA5UnivarDatasetBuilder(LOTSADatasetBuilder):
    dataset_list = [f"era5_univar_{year}" for year in range(1989, 2018 + 1)]
    # dataset_list = [f"era5_univar_{year}" for year in range(1989, 1990)]
    dataset_type_map = defaultdict(lambda: TimeSeriesDataset)
    dataset_load_func_map = defaultdict(lambda: partial(TimeSeriesDataset))
    uniform = True
    ...
    def build_dataset(self, dataset: str, num_proc: int = os.cpu_count()):
        ...
        def gen_func(
            ...
            for v in range(len(ERA5_VARIABLES)):
                for x, y in jobs:
                    yield dict(
                        item_id=f"{year}_{x}_{y}",
                        start=pd.Timestamp(f"{year}-01-01"),
