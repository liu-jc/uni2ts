...
class LibCityUnivarDatasetBuilder(LOTSADatasetBuilder):
    dataset_list = [
        "BEIJING_SUBWAY_30MIN_univar",
        "HZMETRO_univar",
        "LOOP_SEATTLE",
        "LOS_LOOP",
        "M_DENSE",
    ...
    def build_dataset(self, dataset: str, num_proc: int = os.cpu_count()):
        lib_city_path = Path(os.getenv("LIB_CITY_PATH"))
        engine = "pyarrow" if dataset == "Q-TRAFFIC" else None
        if dataset.endswith("univar"):
            raw_dataset = dataset.replace("_univar", "")

        if raw_dataset in ("PEMS03", "PEMS04", "PEMS07", "PEMS08"):
            raw_dataset_name = raw_dataset.replace("0", "D")
        else:
        ...
        def gen_func():
            for dim in range(target_dim):
                for idx in df.entity_id.unique():
                    ...
                    target = entity_df[data_col].to_numpy().astype(np.float32)[:, dim]
                    # if target.ndim == 2:
                    #     if target.shape[-1] == 1:
                    #         target = target.squeeze(-1)
                    #     else:
        ...
        hf_datasets.info.dataset_name = dataset
        hf_datasets.save_to_disk(self.storage_path / dataset)
        print(f"Saved {dataset} to {self.storage_path / dataset}")
