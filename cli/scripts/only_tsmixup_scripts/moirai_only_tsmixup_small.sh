...
for lr in 1e-3
do
  python -m cli.train -cp conf/pretrain --config-name moirai_only_tsmixup run_name=bs-${batch_size}-${max_epoch}-lr${lr}-mskrate${min_mask_ratio}-${max_mask_ratio} model=moirai_small \
