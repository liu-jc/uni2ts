...
do
  ...
  python -m cli.train -cp conf/pretrain --config-name moirai_univar run_name=bs-${batch_size}-${max_epoch}-lr${lr}-mskrate${min_mask_ratio}-${max_mask_ratio} model=moirai_small data=lotsa_v1_univar \
